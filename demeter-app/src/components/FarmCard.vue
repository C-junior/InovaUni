<template>
  <div class="bg-white rounded-xl shadow-soft hover:shadow-medium transition-all duration-200 p-4 sm:p-6 cursor-pointer border border-gray-100 hover:border-primary-200 transform hover:scale-[1.02]"
       @click="handleSelect">
    ...
    <div v-if="calculationStats.count > 0" class="mt-4 p-3 bg-primary-50 rounded-lg border border-primary-100">
      <div class="flex items-center justify-between mb-2">
        ...
        <span class="text-xs text-gray-500">{{ calculationStats.count }} cálculo{{ calculationStats.count > 1 ? 's' : '' }}</span>
      </div>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-xs sm:text-sm">
        ...
        <div>
          ...
          <div class="font-medium text-secondary">{{ formatLastCalculationDate(calculationStats.lastCalculationDate) }}</div>
        </div>
      </div>
    </div>
